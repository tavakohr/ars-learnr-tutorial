...
Every chapter follows a consistent **three-step pedagogical pattern**:
...
- **ARS / ARM-TS / ARD** — the three terms, how they nest, how they differ:
  ...
  - *ARM-TS* (Analysis Results Metadata Technical Specification) = the plan layer:
    populations, methods, analyses, outputs
...
- **Color-coded JSON walkthrough**: a minimal mean-age table rendered
  as blue (ARM-TS) + orange (ARD) sections in a single `ReportingEvent`
...
|----------|-------------|
| 1.1 | Count distinct treatment arms (`TRT01A`) in the Safety population — maps to `GroupingFactor.groups` |
| 1.2 | Count subjects with at least one TEAE by arm — produces the three `OperationResult.rawValue`s for one analysis row |
...
- **AnalysisSet vs DataSubset** (the most-tested distinction):
  AnalysisSet = subject-level population filter (ADSL);
  DataSubset = data-record-level filter (BDS)
- **WhereClause system**: simple condition, compound expression
  (`AND`/`OR`/`NOT`), referenced clause
- Comparator-to-dplyr mapping: `EQ`→`==`, `GE`→`>=`, `IN`→`%in%`, etc.
...
| 2.2 | Translate `AS002` WhereClause (`ADSL.SAFFL EQ 'Y'`) into a `dplyr::filter()` and count subjects |
...
| 2.6 | Pull a single `OperationResult` (mean age, Xanomeline High Dose) and read rawValue + formattedValue |
...
- **Annotated-shell to ARM-TS field mapping** (six-row table, one row
  per annotation element → one ARS field)
- `AnalysisProgrammingCodeTemplate` — the ARS version of a parameterized macro
...
- **`name` / `label` / `description`** — three text fields, when to use each
- **JSON field syntax**: camelCase, `*Id` vs `*Ids`, plural collections,
  mutual-exclusion slot pairs (`condition` OR `compoundExpression`)
- **`resultPattern` grammar**: `X`s after decimal = decimal places
  (`"XXX.X"` → 1 dp; `"XXX.XX"` → 2 dp)
...
| Exercise | What you do |
|----------|-------------|
| 3.1 | Extract `AS002`, reconstruct `"ADSL.SAFFL EQ 'Y'"` with `glue()` — core of `ars_to_shell()` |
...
- Field-by-field flat-ARD → formal-ARD mapping:
  `group1_level` → `groupId`, `stat_name` → `operationId`, `stat` → `rawValue`
...
- **What the ARD cannot tell you alone**: population, data subset, method,
  SAP reference, display context — all require the ARM-TS

**Exercises**

| Exercise | What you do |
|----------|-------------|
| 4.1 | Generate a continuous ARD (`ard_continuous`) for AGE by TRT01A — predict 18 rows (6 stats × 3 arms) |
...
- Where the chain is weakest in industry today (the SAP → ARM-TS gap)
...
- Shell decomposition: one annotated-shell row → six ARM-TS objects
...
| 5.4 | Capstone: decompose a full shell row into six ARS objects (`tribble` mapping exercise) |
...
- The coverage picture: `{cards}` / `{cardx}` → ARD; `{gtsummary}` / `{rtables}` → Display;
  `{datasetjson}` → Exchange; ARM-TS layer → uncovered (the `arsbridge` gap)
...
- `siera` (TransCelerate / commercial): what exists, why an open-source
  equivalent is still missing
- **The gap exercise**: manually apply an `AS002` WhereClause to `adsl`
  with `.data[[cond$variable]]` — this mechanical step is what `parse_ars()` automates
...
### Chapter 7 — Package Design Roadmap: arsbridge (`ars_tutorial_ch7.Rmd`)
...
| `shell_to_ars()` | Shell → ARM-TS | Excel annotated shell | ARM-TS JSON |
| `validate_ard()` | Validate | ARS R list + ARD data frame | Validation report |
| `ars_summary()` | Explore | ARS R list | Console summary |

- **`parse_ars()` internals**: `index_by_id()` — ID-indexed lists for O(1)
  lookups; every object collection indexed by its `id` field
...
- **`validate_ard()` checks 1–6** (same six as Chapter 5)
...
| 7.3 | Prototype the Populations section of `ars_summary()` — one console line per AnalysisSet |
| 7.4 | Prototype `validate_ard()` Check #1 (Coverage) — `%in%` test: every ARM-TS analysis ID in the ARD |
| 7.5 | Capstone: build the `arsbridge_map` `tribble` — each function to its pipeline step and dependencies |
...
- **ADaM datasets** come from `{pharmaverseadam}` — the same data shape used
  ...
  `AGE`, `SEX`; `ADAE` carries `TRTEMFL`, `AEDECOD`; `ADTTE` (via
  `adtte_onco`) carries `PARAMCD`, `AVAL`, `CNSR`.
...
- **Subtitle**: population filter in red — `ADSL.SAFFL='Y'`
- **Row labels**: analysis description + `DATASET.VARIABLE='VALUE'`
  annotations in red inline with the row text
...
makes the shell→ARM-TS translation visible in both directions.
...
Physician · Medical Writer · Senior Clinical Statistical Programmer  
